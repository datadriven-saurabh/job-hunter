...
export default function DeletedOpportunities({onChanged}:{onChanged:()=>Promise<void>}){
 ...
 return <details className="deleted-opportunities" onToggle={e=>{if(e.currentTarget.open)load()}}><summary>Deleted opportunities</summary><p>Removed from your dashboard and application list. Its generated kits, answers and documents are removed. Restore brings back the listing only; create a new kit if needed.</p>{error&&<p role="alert">{error}</p>}{items.map(j=><div key={j.job_id}><span>{j.company_name} · {j.job_title}</span><button className="outline compact" disabled={busy} onClick={async()=>{setBusy(true);try{await api('/applications/restore','POST',{job_ids:[j.job_id]});await load();await onChanged()}catch(e){setError((e as Error).message)}finally{setBusy(false)}}}>Restore {j.company_name}</button></div>)}{!items.length&&<p>No deleted opportunities.</p>}</details>
}
