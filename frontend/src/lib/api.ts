...
export async function api<T = any>(path: string, method = 'GET', body?: unknown): Promise<T> {
  ...
  const timeout=path==='/jobs/search'?90000:60000;
  let timer:ReturnType<typeof setTimeout>;
  const deadline=new Promise<never>((_,reject)=>{timer=setTimeout(()=>{
    controller.abort();
    reject(new Error(path==='/jobs/search'?'Search timed out. You can retry with fewer boards. Some jobs may already have been saved; check your opportunities first.':'The server took too long to respond. Please reconnect and try again.'));
  },timeout)});
  try{return await Promise.race([deadline,(async()=>{
    ...
    const response=await fetch(`${BASE}/api/v1${path}`,{method,signal:controller.signal,headers:{...headers,...(revision?{'If-Match':revision}:{})},...(body!==undefined?{body:JSON.stringify(body)}:{})});
    if(response.status===401&&typeof window!=='undefined'&&hosted)window.location.assign('/login?expired=1');
    if(!response.ok){const data=await response.json().catch(()=>({}));throw new Error(typeof data.detail==='string'?data.detail:`Server request failed (${response.status}). Please retry.`)}
